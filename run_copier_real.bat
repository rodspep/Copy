@echo off
REM Launch the UG copier (DRY-RUN — see run_copier_real_inner.bat) detached, in the MT5
...
REM feed data\ug\live_signals.jsonl. To go LIVE, edit run_copier_real_inner.bat to add
...
echo [copier-REAL] launching in session %SID% (see logs\copier.log)...

"%BOTDIR%\PsExec64.exe" -accepteula -nobanner -i %SID% -d cmd /c "cd /d ""%BOTDIR%"" && call ""%BOTDIR%\run_copier_real_inner.bat"" >> ""%BOTDIR%\logs\copier.log"" 2>&1"
echo [copier-REAL] launched (PsExec exit %errorlevel%).
endlocal
